...
class ActorNetwork:
    ...
    def create_network(self, state_dim, action_dim):
        ...
        action_linear = self.batch_norm_layer(action[:, None, 0], training_phase=is_training, scope_bn='action_linear',
                                              activation=tf.sigmoid)
        action_angular = self.batch_norm_layer(action[:, None, 1], training_phase=is_training,
                                               scope_bn='action_angular', activation=tf.tanh)
    ...
    def create_target_network(self, state_dim, action_dim, net):
        ...
        action_linear = self.batch_norm_layer(action[:, None, 0], training_phase=is_training,
                                              scope_bn='target_action_linear', activation=tf.sigmoid)
        action_angular = self.batch_norm_layer(action[:, None, 1], training_phase=is_training,
                                               scope_bn='target_action_angular', activation=tf.tanh)
        # action_linear = tf.sigmoid(action[:, None, 0])
        # action_angular = tf.tanh(action[:, None, 1])
        action = tf.concat([action_linear, action_angular], axis=-1)
    ...
    def target_actions(self, state_batch):
        return self.sess.run(self.target_action_output,
                             feed_dict={self.target_state_input: state_batch, self.target_is_training: True})
